-- Idempotent RLS policy repair for all Seldom tables.
-- Safe to re-run anytime. Run via: npm run supabase:push
-- Or paste supabase/apply-all-policies.sql in SQL Editor for immediate fix.
--
-- NOTE: "policy ... does not exist, skipping" on DROP POLICY IF EXISTS is a
-- normal PostgreSQL NOTICE on first run — not an error. CREATE POLICY runs next.
