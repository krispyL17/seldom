-- Seldom: college module tables only
-- Safe to re-run. Run in SQL Editor if verify-tables.sql shows MISSING college tables.
-- Requires set_updated_at() — run apply-all.sql first if you get a function error.
...
create table if not exists public.college_activities (
  ...
  organization text, role text, description text,
  ...
  leadership text, achievements text,
...
create trigger college_activities_set_updated_at before update on public.college_activities for each row execute function public.set_updated_at();
...
create table if not exists public.college_awards (
  ...
  user_id uuid not null references auth.users (id) on delete cascade,
  name text not null check (char_length(trim(name)) > 0),
  organization text, award_date date, level text, description text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create trigger college_awards_set_updated_at before update on public.college_awards for each row execute function public.set_updated_at();
...
create table if not exists public.college_projects (
  ...
  my_role text, results text, challenges text, lessons_learned text,
  documents jsonb not null default '[]'::jsonb,
  start_date date, end_date date,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
create trigger college_projects_set_updated_at before update on public.college_projects for each row execute function public.set_updated_at();
...
create trigger college_user_data_set_updated_at before update on public.college_user_data for each row execute function public.set_updated_at();
