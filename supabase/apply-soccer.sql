-- Seldom: soccer module tables (coach + profile)
-- Safe to re-run. Run in SQL Editor if verify-tables.sql shows MISSING soccer tables.
-- Requires set_updated_at() — run apply-all.sql first if you get a function error.
-- After manual apply, run: npm run supabase:repair
...
create trigger soccer_matches_set_updated_at before update on public.soccer_matches for each row execute function public.set_updated_at();
...
create trigger soccer_insights_set_updated_at before update on public.soccer_insights for each row execute function public.set_updated_at();
...
create policy "Authenticated users can read active prompts" on public.ai_prompts for select using (auth.role() = 'authenticated' and active = true);
...
create trigger soccer_user_data_set_updated_at before update on public.soccer_user_data for each row execute function public.set_updated_at();
